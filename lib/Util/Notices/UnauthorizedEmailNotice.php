...
class UnauthorizedEmailNotice {
  ...
  private function getMessageText($validation_error) {
    if (
      ...
      $text = $this->wp->x('<b>Sending all of your emails has been paused</b> because your email address %email-address hasn’t been authorized yet.</b>',
  ...
  private function getSettingsButtons($validation_error) {
    ...
    if (!empty($validation_error['invalid_sender_address'])) {
      $button = $this->wp->x('Update my Default sender email address', 'Please reuse the current translation of “Default sender”');
    ...
    if (!empty($validation_error['invalid_confirmation_address'])) {
      $button = $this->wp->x('Update my Sign-up Confirmation email address', 'Please reuse the current translation of “Sign-up Confirmation”');
  ...
  private function getAuthorizationLink($validation_error) {
    if (
      ...
      $email = isset($validation_error['invalid_sender_address']) ? $validation_error['invalid_sender_address'] : $validation_error['invalid_confirmation_address'];
      $authorize_link = $this->wp->x('Authorize %email', 'Link for user to authorize their email address');
