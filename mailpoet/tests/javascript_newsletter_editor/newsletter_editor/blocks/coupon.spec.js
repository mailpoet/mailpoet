...
describe('Coupon', function () {
  ...
  describe('block settings view', function () {
    ...
    it('renders', function () {
      var view = new CouponBlock.CouponBlockSettingsView({ model: model });
      expect(view.render).to.not.throw();
    });
    ...
    describe('once rendered', function () {
      ...
      beforeEach(function () {
        model = new CouponBlock.CouponBlockModel();
        view = new CouponBlock.CouponBlockSettingsView({ model: model });

        view.render();
      });

      it('updates the model when discount type is changed', function () {
        var newValue = 'fixed_product';
        view
          .$('.mailpoet_field_coupon_discount_type')
          .val(newValue)
          .trigger('change');
        expect(model.get('discountType')).to.equal(newValue);
      });

      it('updates the model when amount is changed', function () {
        var newValue = '15';
        view.$('.mailpoet_field_coupon_amount').val(newValue).trigger('input');
        expect(model.get('amount')).to.equal(newValue);
      });

      it('updates the model when expiry day is changed', function () {
        var newValue = '5';
        view
          .$('.mailpoet_field_coupon_expiry_day')
          .val(newValue)
          .trigger('input');
        expect(model.get('expiryDay')).to.equal(newValue);
      });

      it('updates the model when text alignment is changed', function () {
        view
          .$('.mailpoet_field_coupon_alignment[value="right"]')
          .prop('checked', true)
          .trigger('change');
        expect(model.get('styles.block.textAlign')).to.equal('right');
      });

      it('updates the model when font color is changed', function () {
        var newValue = '#ccc';
        view
          .$('.mailpoet_field_coupon_font_color')
          .val(newValue)
          .trigger('change');
        expect(model.get('styles.block.fontColor')).to.equal(newValue);
      });

      it('updates the model when font family is changed', function () {
        var newValue = 'Arial';
        view
          .$('.mailpoet_field_coupon_font_family')
          .val(newValue)
          .trigger('change');
        expect(model.get('styles.block.fontFamily')).to.equal(newValue);
      });

      it('updates the model when font weight is changed', function () {
        var newValue = 'bold';
        view
          .$('.mailpoet_field_coupon_font_weight')
          .val(newValue)
          .trigger('click');
        expect(model.get('styles.block.fontWeight')).to.equal(newValue);
      });

      it('updates the model when font size is changed', function () {
        var newValue = '20px';
        view
          .$('.mailpoet_field_coupon_font_size')
          .val(newValue)
          .trigger('change');
        expect(model.get('styles.block.fontSize')).to.equal(newValue);
      });

      it('updates the model when background color is changed', function () {
        var newValue = '#123456';
        view
          .$('.mailpoet_field_coupon_background_color')
          .val(newValue)
          .trigger('change');
        expect(model.get('styles.block.backgroundColor')).to.equal(newValue);
      });

      it('updates the model when border color is changed', function () {
        var newValue = '#cceeaa';
        view
          .$('.mailpoet_field_coupon_border_color')
          .val(newValue)
          .trigger('change');
        expect(model.get('styles.block.borderColor')).to.equal(newValue);
      });

      it('updates the model when border radius is changed', function () {
        var newValue = '3';
        view
          .$('.mailpoet_field_coupon_border_radius_input')
          .val(newValue)
          .trigger('input');
        expect(model.get('styles.block.borderRadius')).to.equal(
          newValue + 'px',
        );
      });

      it('updates the model when border width is changed', function () {
        var newValue = '4';
        view
          .$('.mailpoet_field_coupon_border_width_input')
          .val(newValue)
          .trigger('input');
        expect(model.get('styles.block.borderWidth')).to.equal(newValue + 'px');
      });

      it('updates the model when width is changed', function () {
        var newValue = '250';
        view
          .$('.mailpoet_field_coupon_width_input')
          .val(newValue)
          .trigger('input');
        expect(model.get('styles.block.width')).to.equal(newValue + 'px');
      });

      it('updates the model when height is changed', function () {
        var newValue = '15';
        view
          .$('.mailpoet_field_coupon_line_height_input')
          .val(newValue)
          .trigger('input');
        expect(model.get('styles.block.lineHeight')).to.equal(newValue + 'px');
      });
    });
  });
});
