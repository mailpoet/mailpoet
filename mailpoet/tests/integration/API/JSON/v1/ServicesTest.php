...
class ServicesTest extends \MailPoetTest {
  ...
  public function testItReplacesAnEarlierPauseReasonWhenAFreshKeyBecomesPendingApproval() {
    $this->settings->set(Mailer::MAILER_CONFIG_SETTING_NAME, ['method' => Mailer::METHOD_MAILPOET]);
    (new SettingsFactory())->withSendingError('bridge unavailable', MailerError::OPERATION_SEND);
    verify(MailerLog::isSendingPaused())->true();

    $pendingBridge = $this->make(
      Bridge::class,
      [
        'settings' => SettingsController::getInstance(),
    ...
    $this->createServicesEndpointWithMocks(['bridge' => $pendingBridge])->checkMSSKey($this->data);
    verify(MailerLog::isSendingPaused())->true();
    verify(MailerLog::getError()['operation'] ?? null)->equals(MailerError::OPERATION_PENDING_APPROVAL);

    $approvedBridge = $this->make(
      Bridge::class,
      [
        'settings' => SettingsController::getInstance(),
    ...
    $this->createServicesEndpointWithMocks(['bridge' => $approvedBridge])->checkMSSKey($this->data);
    verify(MailerLog::isSendingPaused())->false();
  }
  ...
  public function testItResumesSendingPausedForPendingApprovalWhenMSSKeyBecomesInvalid() {
    $this->settings->set(Mailer::MAILER_CONFIG_SETTING_NAME, ['method' => Mailer::METHOD_MAILPOET]);
    $this->settings->set(Bridge::API_KEY_SETTING_NAME, 'key');
    $this->settings->set(Bridge::API_KEY_STATE_SETTING_NAME, [
      'state' => Bridge::KEY_VALID,
