...
class SendingTest extends \MailPoetTest {
  ...
  public function testItSavesSubscriberError() {
    ...
    expect($taskSubscriber->getError())->equals($error);
    expect($taskSubscriber->getFailed())->equals(ScheduledTaskSubscriberEntity::FAIL_STATUS_FAILED);
    expect($taskSubscriber->getProcessed())->equals(ScheduledTaskSubscriberEntity::STATUS_PROCESSED);
    expect($this->sending->count_total)->equals(2);
    expect($this->sending->count_processed)->equals(1);
    expect($this->sending->count_to_process)->equals(1);
