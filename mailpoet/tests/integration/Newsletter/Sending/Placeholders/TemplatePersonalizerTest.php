...
class TemplatePersonalizerTest extends \MailPoetTest {
  ...
  public function testItReplacesTagValuesWithPlaceholdersAndRecordsThemPerPart(): void {
    ...
    $this->assertSame('Hi {{mailpoet_mss_test_1}}', $subject);
    $this->assertStringContainsString('<p>{{mailpoet_mss_test_2}}</p>', $html);
    $this->assertStringContainsString('href="{{mailpoet_mss_test_3}}"', $html);
    ...
    $this->assertSame('{{mailpoet_mss_test_4}} [Review](http://[mailpoet/test-url%5D)', $text);
    $this->assertSame([
      'subject' => ['{{mailpoet_mss_test_1}}' => 'Rosta & Co'],
      'html' => [
        '{{mailpoet_mss_test_2}}' => 'Rosta & Co',
        '{{mailpoet_mss_test_3}}' => 'https://example.com/review-order/abc?email=rosta%40example.com&#038;source=mss',
      ],
      'text' => ['{{mailpoet_mss_test_4}}' => 'Rosta & Co'],
    ], $collector->getValues());
  ...
  public function testItKeepsTemplateIdenticalWhenOneTokenIsUsedAsTextAndLink(): void {
    ...
    $this->assertSame(2, substr_count($plain, '{{mailpoet_mss_ns_'));
  ...
  public function testItNeutralizesAClosingTitleTagInTitleValues(): void {
    ...
    $this->assertStringContainsString('<title>{{mailpoet_mss_ns_1}}</title>', $html);
    $this->assertReconstructsRenderedOutput($source, $html, $collector, PlaceholderCollector::PART_HTML);
  ...
  public function testItKeepsPlaceholdersApartBeyondTenLinks(): void {
    ...
    $this->assertStringContainsString('href="{{mailpoet_mss_ns_1}}"', $html);
    $this->assertStringContainsString('href="{{mailpoet_mss_ns_10}}"', $html);
    $this->assertStringContainsString('href="{{mailpoet_mss_ns_11}}"', $html);
    $this->assertStringNotContainsString('{{mailpoet_mss_ns_1}}0', $html);
