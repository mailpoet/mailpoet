...
class LinksTest extends \MailPoetTest {
  ...
  public function testItReplacesHashedLinksWithPlaceholdersStandingForTrackingUrls() {
    ...
    verify($result)->equals('<a href="{{mailpoet_mss_ns_1}}">one</a><a href="{{mailpoet_mss_ns_1}}">again</a><img src="{{mailpoet_mss_ns_2}}"/>');
    $values = $collector->getValues()['html'];
    verify($values['{{mailpoet_mss_ns_1}}'])->stringContainsString('endpoint=track&action=click');
    verify($values['{{mailpoet_mss_ns_2}}'])->stringContainsString('endpoint=track&action=open');
  ...
  public function testItGivesUntrackedPlaceholdersTheSameShapeAsTrackedOnes() {
    ...
    verify($values['html']['{{mailpoet_mss_ns_1}}'])->equals(OpenTracking::UNTRACKED_PIXEL_SRC);
    verify($values['html']['{{mailpoet_mss_ns_2}}'])->equals('http://example.com/page?untracked');
    verify($values['html']['{{mailpoet_mss_ns_3}}'])->equals('[mailpoet_click_data]-nope');
    verify($values['text']['{{mailpoet_mss_ns_4}}'])->equals('http://example.com/page?untracked');
    verify($values['text']['{{mailpoet_mss_ns_5}}'])->equals('[mailpoet_open_data]');
