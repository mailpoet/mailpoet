...
class MailPoet implements MailerMethod {
  ...
  public function send($newsletter, $subscriber, $extraParams = []): array {
    ...
    $result = $newsletter instanceof TemplateBatch
      ? $this->api->sendTemplateMessages($messageBody)
      : $this->api->sendMessages($messageBody);
  ...
  /**
   * @param array<string, mixed>|array<int, array<string, mixed>>|TemplateBatch $newsletter A rendered email, one per recipient for bulk, or one template for the batch
   * @param array<string, mixed>|string|array<int, mixed> $subscriber
   */
  ...
  private function composeTemplateBatchBody(TemplateBatch $batch, array $subscribers, array $extraParams): array {
    ...
    // map. A mismatch would ship raw {{mp_mss_*}} placeholders, so fail loudly.
    ...
    for ($record = 0; $record < count($subscribers); $record++) {
      ...
      $message = [
        ...
        // The sending service replaces these once and does not expand values again.
