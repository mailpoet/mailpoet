...
class Newsletter {
   ...
   *
   * @param string $contentSource Content that shortcodes such as [newsletter:post_title] look their value up in
   * @param array<string, mixed>|null $personalizationContext
   */
  private function resolveUntrackedUrl(
    ...
    return (string)ShortcodesTask::process($url, $contentSource, $newsletter, $subscriber, $queue);
  }
  ...
  public function prepareNewsletterForTemplatedSending(
    ...
    // Shortcodes such as [newsletter:post_title] look their value up in the whole email, so every
    // part is processed against the same joined source as in prepareNewsletterForSending().
    $contentSource = Helpers::joinObject(array_values($parts));
    foreach ($parts as $part => $content) {
      $parts[$part] = (string)ShortcodesTask::processWithPlaceholders($content, $contentSource, $newsletter, $subscriber, $queue, $collector, $part);
    }
    ...
    if ($this->trackingEnabled) {
      if ($this->trackingConsentController->isTrackingAllowed($subscriber)) {
        ...
        $resolveUntrackedUrl = function (string $url) use ($contentSource, $newsletter, $subscriber, $queue, $context): string {
          return $this->resolveUntrackedUrl($url, $contentSource, $newsletter, $subscriber, $queue, $context);
        };
