    ...
    foreach ($subscribers as $subscriber) {
      ...
      if ($useTemplatedBatch) {
        ...
        if ($templateBatch === null) {
          ...
          $task->setStatus(ScheduledTaskEntity::STATUS_PAUSED);
          $this->scheduledTasksRepository->flush();
          $this->loggerFactory->getLogger(LoggerFactory::TOPIC_SENDING)->error('templated batch template mismatch', [
            'task_id' => $task->getId(),
            'newsletter_id' => $newsletter->getId(),
            'subscriber_id' => $subscriber->getId(),
            'differing_parts' => $this->getDifferingTemplateParts($templateBatch->getTemplate(), $templatedNewsletter['newsletter']),
          ]);
