...
class SendingThrottlingHandler {
  ...
  private function getMaxBatchSize(): int {
    if ($this->useTemplatedSending) {
      ...
      // The rendered-batch filter is not applied here: its values are tuned for a few dozen
      // fully rendered messages, not for a shared template. Receives the ceiling after the
      // server-advertised cap and can only lower it.
