...
class API {
  ...
  // A templated batch carries up to TEMPLATED_BATCH_SIZE substitution maps and the service fans
  // them out before answering, which does not fit the 10 s the plain messages endpoint gets.
  const TEMPLATE_BATCH_REQUEST_TIMEOUT = 60; // seconds
  ...
  // Endpoints registered directly on the WPCOM mailpoet-bridge plugin, not proxied
  // through bridge.mailpoet.com like the v0 ones. Authenticated with the same
  ...
  // Templated batch sending, see sendTemplateMessages(). urlMessages stays on
