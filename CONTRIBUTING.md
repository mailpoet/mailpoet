...
Development tasks are exposed as `pnpm` scripts at the repo root (`pnpm env:start`, `pnpm qa`, `pnpm test:integration`, …). The plugin-level `./do` runner is still used for host-side Robo tasks inside `mailpoet/`. See [README](README.md) for the full command reference.
...
- Code formatting is handled automatically by Prettier. Run `pnpm qa:fix` to format your code.
...
- Use `pnpm qa` to check your code style before pushing.
- Run `pnpm qa:fix` to format your code with Prettier.
