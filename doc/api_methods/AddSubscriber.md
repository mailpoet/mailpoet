...
| tracking_consent (optional)      | string/null         | 20 chars  | Whether the subscriber agreed to email open and click tracking. One of `granted`, `denied`, `unknown`. Use this when your own signup or preference UI collected the answer, since MailPoet's built-in consent checkboxes cannot reach a caller of this API. Any other value is ignored and the subscriber's consent is left as it was. Not affected by the site's "Subscriber choice" setting, so a decline is always honoured. |
| tracking_consent_copy (optional) | string/null         | 65K chars | The exact wording your UI showed when the subscriber answered, stored as proof of what they agreed to. Only used alongside `tracking_consent`. Send the wording you actually displayed: if you omit it, MailPoet stores its own default wording instead, which is weaker evidence because it does not show what your UI said.                                                                                                   |
