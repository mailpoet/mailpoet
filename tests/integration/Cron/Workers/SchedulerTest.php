...
class SchedulerTest extends \MailPoetTest {
  ...
  public function testItThrowsExceptionWhenExecutionLimitIsReached() {
    try {
      $scheduler = new Scheduler($this->makeEmpty(SubscribersFinder::class), $this->loggerFactory, $this->cronHelper, $this->scheduledTasks);
  ...
  public function testItProcessesScheduledStandardNewsletter() {
    ...
    assert($newsletter instanceof Newsletter);
    $queue = $this->_createQueue($newsletter->id);
    $scheduler = new Scheduler($this->subscribersFinder, $this->loggerFactory, $this->cronHelper, $this->scheduledTasks);
  ...
  public function testItFailsToProcessPostNotificationNewsletterWhenSubscribersNotInSegment() {
    ...
    $scheduler = $this->construct(Scheduler::class, [$this->subscribersFinder, $this->loggerFactory, $this->cronHelper, $this->scheduledTasks], [
      'deleteQueueOrUpdateNextRunDate' => Expected::exactly(1, function() {
        return false;
      }),
    ]);
  ...
  public function testItDoesNotReSchedulesBounceTaskWhenSoon() {
    ...
    $scheduler = new Scheduler($finder, $this->loggerFactory, $this->cronHelper, $this->scheduledTasks);
